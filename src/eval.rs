    ...
    fn test_eval_rem() {
        ...
        for _ in 0..100 {
            ...
            if b.to_f64() > 0.0 {
                ...
                let q = Fixed(raw_q);
                let r = Fixed(raw_r);
                test_op_internal(Op::Rem, &vec![a, b], &vec![q, r], 3);
            }
        }
        ...
        for (a0, b0) in special_cases {
            let fixed_a = Fixed::<15>::from_f64(a0);
            let fixed_b = Fixed::<15>::from_f64(b0);
            if fixed_b.to_f64() > 0.0 {
                let raw_q = fixed_a.0.div_euclid(fixed_b.0);
                let raw_r = fixed_a.0.rem_euclid(fixed_b.0);
                let q = Fixed(raw_q);
                let r = Fixed(raw_r);
                test_op_internal(Op::Rem, &vec![fixed_a, fixed_b], &vec![q, r], 3);
            }
        }
    }
}
